...
- Tauri 桌面打包：新增 `src-tauri`、手动 `Tauri builds` workflow 和正式 `Release` workflow，需验证手动触发能分别产出 macOS Apple Silicon / Intel `.dmg`、Windows `.exe`、Linux `.AppImage` / `.deb`，并验证 `v*.*.*` tag 发布会创建草稿 Release、完成 Apple 签名 / notarization 后发布正式 Tauri 产物。
