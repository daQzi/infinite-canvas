...
- 桌面应用打包：新增 Electron 桌面壳和 `Desktop builds` GitHub Actions workflow，需验证手动触发能分别产出 macOS `.dmg` / `.zip`、Windows `.exe`、Linux `.AppImage` / `.deb`。
...
- GitHub Actions 触发策略：仅 `Release` workflow 保留 `v*.*.*` 标签自动触发；Desktop、Tauri、Docker、Docs Docker、GitHub Pages 和 Canvas Agent 发布均只允许手动触发，需验证 Actions 页面可正常启动各工作流且推送普通提交或标签不会重复触发。
