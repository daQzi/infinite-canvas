...
- 桌面应用打包：新增 Electron 桌面壳和 `Desktop builds` GitHub Actions workflow，需验证手动触发和 `v*` tag 触发能分别产出 macOS `.dmg` / `.zip`、Windows `.exe`、Linux `.AppImage` / `.deb`，并确认未签名包的系统提示符合预期。
- Tauri 桌面打包：新增 `src-tauri` 和 `Tauri builds` GitHub Actions workflow，需验证手动触发和 `v*` tag 触发能分别产出 macOS `.dmg`、Windows `.exe`、Linux `.AppImage` / `.deb`，并对比 Electron 版本包体积和运行兼容性。
