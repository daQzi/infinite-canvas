...
- Tauri 原生下载：图片、视频、音频、素材 ZIP 和画布 ZIP 会打开系统“另存为”窗口；需在 macOS / Windows 验证保存后文件可正常打开，取消保存不会报错，Web/Docker 下载行为保持不变。
- Tauri 桌面打包：手动 `Tauri builds` workflow 仍需验证 macOS Apple Silicon / Intel `.dmg`、Windows `.exe`、Linux `.AppImage` / `.deb`；`v*.*.*` 标签触发的 `Release` workflow 仅构建 macOS 和 Windows 安装包并创建草稿 Release，配置 Apple secrets 时会签名 / notarization，未配置时会发布未签名 macOS 产物。
- 本地 Agent 连接说明：新增提醒说明只有安装 Codex 插件或手动添加 MCP 后才会增加 Codex token 消耗，直接运行 `npx -y @basketikun/canvas-agent` 不会安装 MCP；需验证文案位置清晰。
- 本地 Agent 连接说明：画布面板和配置弹窗 Codex Tab 改为区分“Codex 插件启动画布”和“直接运行 Agent 后网页连接”两种方式，需验证文案和布局清晰。
- 全站 Agent：Agent 面板从画布页抽离为全站常驻面板，挂在全局布局右侧，开/关只挤压左侧内容不遮挡；顶栏和画布工具条各有一个开关按钮，需验证在首页、生图、视频、素材、提示词库、画布等页面都能打开面板并保持 SSE 连接不断开。
- 全站 Agent：新增 `site_navigate` 工具，Agent 可直接跳转 `/`、`/canvas`、`/canvas/:id`、`/image`、`/video`、`/prompts`、`/assets`，传入 `/config` 时打开统一配置弹窗；导航直接执行不走工具确认，需验证跳转或弹窗正确且 Agent 面板不消失。
...
- GitHub Actions 触发策略：仅 `Release` workflow 保留 `v*.*.*` 标签自动触发；Tauri、Docker、Docs Docker、GitHub Pages 和 Canvas Agent 发布均只允许手动触发，需验证 Actions 页面可正常启动各工作流且推送普通提交或标签不会重复触发。
- Docker AI 代理：主应用 nginx 镜像新增 `/v1/` 同源代理，`AI_PROXY_TARGET` 可配置上游 OpenAI 兼容地址；需验证浏览器 Base URL 填当前站点地址时，可以避开上游 CORS 限制。
...
- 配置与用户偏好：统一使用全局配置弹窗，移除独立顶部导航页面；弹窗保留 Codex Tab、本地 Agent 地址、Connect token 和工具执行确认开关，完整配置点击“完成”后提示保存成功，不完整配置保持弹窗打开并提示缺项。
- Codex 全局连接：配置弹窗可直接连接本地 Canvas Agent，连接状态同步到画布和顶部 OpenAI 状态图标，刷新后会按本地保存的地址和 token 自动尝试重连；需验证已连接、连接中、连接失败、断开和刷新自动重连状态。
- 顶部外部链接：普通页面、画布右上角和画布菜单移除文档及 GitHub 外部入口，需确认剩余配置、主题、版本和快捷键控件布局正常。
