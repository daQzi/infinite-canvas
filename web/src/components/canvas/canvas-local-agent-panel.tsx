        ...
        try {
            ...
            if (payload.name === "site_navigate") {
                ...
                if (path === "/config") {
                    ...
                    result = { ok: true, path, opened: "config-dialog" };
                } else {
                    navigate(path);
                    result = { ok: true, path };
                }
            ...
            setAgentState({ activity: "工具完成", waiting: true });
            addEventLog(`${toolName(payload.name)}完成`, result, result);
            addMessage({ role: "tool", title: `${toolName(payload.name)}完成`, text: payload.name === "canvas_apply_ops" ? summarizeCanvasAgentOps(input.ops || []) || "画布操作" : payload.name === "site_navigate" ? (input.path === "/config" ? "已打开配置弹窗" : `已跳转到 ${input.path || "/"}`) : "已完成", detail: { requestId: payload.requestId, name: payload.name, input, result } });
        } catch (error) {
