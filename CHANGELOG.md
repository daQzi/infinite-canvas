...
+ [修复] 修复 Tauri 原生下载参数字段无法反序列化的问题，并保留 Rust 返回的具体保存错误。

## v0.6.4 - 2026-07-11

+ [修复] Tauri 桌面端图片、视频、音频和 ZIP 下载改用系统原生“另存为”窗口，修复下载无法落盘的问题。
...
+ [修复] 调整 streamdown 样式依赖的导入顺序，避免 PostCSS 忽略后续样式。
+ [新增] Agent 新增画布、工作台、提示词库和素材等站点级工具，支持读取配置、触发生成与新增素材。
+ [新增] Agent 面板改为全站常驻右侧栏，开关时同步推动顶栏和页面内容，并保留独立入口按钮。
+ [新增] Agent 新增 `site_navigate` 工具，支持从对话中直接跳转首页、画布、工作台、素材，并打开统一配置弹窗。
...
+ [修复] 生图工作台重新打开失败记录时恢复失败结果卡片，并保留失败原因。

## v0.6.2 - 2026-07-10

+ [调整] Tauri Rust 层重构为本地后端模块，统一承接桌面端 AI HTTP 请求与媒体文件落盘存储。
+ [调整] 配置与用户偏好统一使用全局弹窗，移除独立配置页面及应用内文档、GitHub 外部链接。
+ [调整] GitHub Actions 仅保留 Release 的版本标签自动触发，其余工作流统一改为手动触发。
+ [调整] Release workflow 移除 Linux 安装包构建，仅发布 macOS 和 Windows 安装包。
...
+ [修复] Docker 本地运行新增 AI 接口代理，兼容不开放 CORS 的中转站。
+ [修复] Tauri 桌面端 AI 请求改由 Rust 原生 HTTP 执行，避免 WebView CORS 拦截。
